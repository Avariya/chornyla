...
function flattenCubic(
  ...
  flattenCubic(x0, y0, (x0 + x1) / 2, (y0 + y1) / 2, (x0 + 2 * x1 + x2) / 4, (y0 + 2 * y1 + y2) / 4, mx, my, tolerance, points);
  flattenCubic(mx, my, (x1 + 2 * x2 + x3) / 4, (y1 + 2 * y2 + y3) / 4, (x2 + x3) / 2, (y2 + y3) / 2, x3, y3, tolerance, points);
}
...
function glyphToPoints(glyph: PositionedGlyph, pageHeight: number): number[][][] {
  ...
  let cx = 0, cy = 0; // current position in font units
